...
if (!([bool]([Environment]::GetCommandLineArgs() -like '*-noninteractive*'))) {
    ...
    $antminerUser = Read-Host -Prompt "Press enter to accept the default Antminer User mharris867.027" # Prompt for Antminer user
...
if ([string]::IsNullOrWhiteSpace($antminerUser)) { $antminerUser = "mharris867.027" } # Set Antminer User to default if not input from user
...
While ($loopforever) {
    ...
    if ((Invoke-RestMethod http://$antminerIP/cgi-bin/get_miner_status.cgi -Credential $creds -AllowUnencryptedAuthentication).summary.ghs5s -lt 32) {
        ...
        $body = "_ant_pool1url=stratum%2Btcp%3A%2F%2Fequihash.f2pool.com%3A5100&_ant_pool1user=$antminerUser&_ant_pool1pw=x&_ant_pool2url=stratum%2Btcp%3A%2F%2Fequihash.f2pool.com%3A5100&_ant_pool2user=$antminerUser&_ant_pool2pw=x&_ant_pool3url=&_ant_pool3user=&_ant_pool3pw=&_ant_nobeeper=false&_ant_notempoverctrl=false&_ant_fan_customize_switch=false&_ant_fan_customize_value=&_ant_freq=500"
